...
class ReceiptSorterApp:
    ...
    def validate_setup(self) -> bool:
        ...
        if not receipt_files:
            logger.warning(f"No receipt files found in {self.source_folder}")
            print(f"\n⚠️  Warning: No receipt files found in {self.source_folder}")
            print(f"Please add PDF or image receipts to this folder and run again.")
            return False
        ...
        if not config.GOOGLE_SHEET_ID:
            logger.warning("GOOGLE_SHEET_ID not set in environment or config")
            print(f"\n⚠️  Warning: GOOGLE_SHEET_ID not set")
            print(f"Google Sheets integration will be disabled. Set GOOGLE_SHEET_ID in .env")

        if not os.path.exists(config.GOOGLE_SERVICE_ACCOUNT_FILE):
            logger.warning(f"Google service account file not found: {config.GOOGLE_SERVICE_ACCOUNT_FILE}")
            print(f"\n⚠️  Warning: {config.GOOGLE_SERVICE_ACCOUNT_FILE} not found")
            print(f"Google Sheets integration will be disabled.")

        print(f"✓ Found {len(pdf_files)} PDF receipt(s) to process")
        return True

    def get_receipt_files(self) -> List[str]:
        """
        Get list of receipt files (PDF and images) in source folder
    ...
    def process_receipt(self, file_path: str) -> bool:
        """
        ...
        try:
            ...
            if not self.pdf_processor.validate_file(file_path):
                logger.error(f"Invalid or corrupted file: {filename}")
                print(f"   ❌ Invalid or corrupted file")
                self.stats['failed'] += 1
                return False

            # Step 2: Extract text from file
            print(f"   📖 Extracting text...")
            receipt_text = self.pdf_processor.extract_text(file_path)
            ...
            if not spreadsheet_success:
                logger.warning(f"Failed to update local spreadsheet for {filename}")
                print(f"   ⚠️  Could not update local spreadsheet")

            # Step 8: Update Google Sheets
            print(f"   ☁️  Updating Google Sheets...")
            gs_success = self.google_sheets_manager.add_receipt_entry(
                receipt_data,
                category,
                confidence,
                organized_path
            ...
            if not gs_success:
                logger.warning(f"Failed to update Google Sheets for {filename}")
                print(f"   ⚠️  Could not update Google Sheets (check credentials/Sheet ID)")
    ...
    def run(self):
        ...
        receipt_files = self.get_receipt_files()
        self.stats['total_processed'] = len(receipt_files)

        print(f"\n🚀 Starting processing of {len(receipt_files)} receipt(s)...")
        print(f"   Source: {self.source_folder}")
        print(f"   Output: {self.output_folder}")

        # Process each receipt
        for idx, file_path in enumerate(receipt_files, 1):
            print(f"\n{'─' * 70}")
            print(f"Receipt {idx} of {len(receipt_files)}")
